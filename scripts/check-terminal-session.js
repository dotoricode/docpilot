...
(async () => {
  ...
  try {
    ...
    const created = await requestJson(port, 'POST', '/terminal-sessions', { agent: 'claude' });
    assert.strictEqual(created.session.agent, 'claude');
    ...
    const eventsPromise = collectTerminalEvents(port, created.session.id, events =>
      events.some(event => event.type === 'terminal.data' && String(event.data || '').includes('fake claude> hello terminal')),
    );
    ...
    assert(events.some(event => event.type === 'terminal.data' && String(event.data || '').includes('fake claude interactive ready')), 'initial terminal output missing');
    assert(events.some(event => event.type === 'terminal.data' && String(event.data || '').includes('fake claude> hello terminal')), 'terminal input response missing');
