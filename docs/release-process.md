...
6. macOS 26 이상에서 앱 아이콘은 Apple Icon Composer `.icon` 자산을 사용하고, 레거시 호환용 및 DMG volume용 `.icns`는 별도로 유지합니다. `.icns`만 앱 아이콘으로 지정하면 Finder가 밝거나 어두운 시스템 플레이트를 추가할 수 있으므로 허용하지 않습니다.
7. 매뉴얼 계약과 production build를 실행합니다.
...
`check-packaged-app.js`는 x64/arm64 DMG를 각각 마운트해 bundle version과 앱/volume ICNS의 alpha를 검사합니다. 앱 bundle에 `CFBundleIconName=Icon`과 `Assets.car`가 포함됐는지 확인하고, host 아키텍처 앱을 macOS `NSWorkspace`로 실제 렌더해 바깥 시스템 플레이트가 밝게 합성되지 않는지도 검사합니다. 직접 Finder에서도 같은 결과를 확인합니다.
...
- 앱 bundle이 Icon Composer `Assets.car`와 `CFBundleIconName`을 사용하며, 레거시 `.icns`만으로 앱 아이콘을 제공하지 않는가
