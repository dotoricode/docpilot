...
const guidePages = {
  overview: guide({
    ...
    steps: ['프로젝트 폴더를 열어 기본 작업공간을 만듭니다.', 'Project 패널에서 문서를 선택해 탭으로 엽니다.', 'Source, Document, Preview 또는 Tree 중 파일 형식에 제공되는 보기를 선택합니다.', '변경을 확인할 때 Diff를 켜고, 프로젝트 명령은 Terminal에서 실행합니다.'],
  ...
  install: guide({
    description: '팀 내부 공유용 macOS DMG를 내려받아 Applications에 설치합니다. 사이트의 Download에서 Apple Silicon 또는 Intel Mac을 선택할 수 있습니다.',
    outcome: 'unsigned 앱을 최초 1회 승인하고 DocPilot의 폴더 선택 화면까지 도달합니다.',
    before: ['macOS에서 앱을 설치할 권한이 필요합니다.', '기존 버전을 사용 중이면 열려 있는 문서를 저장하세요.', 'GitHub Release의 버전과 SHA-256을 확인하고 신뢰하는 팀 배포본에만 보안 예외를 허용하세요.'],
    steps: ['상단 Download를 누르고 사용 중인 Mac의 칩 또는 프로세서를 선택합니다.', 'DMG를 열고 DocPilot을 Applications 폴더로 옮깁니다.', 'Applications에서 DocPilot을 한 번 실행합니다. unsigned 빌드이므로 macOS가 악성 코드 확인 불가 경고로 실행을 차단하는 것이 정상입니다.', '경고에서 완료를 누르고 시스템 설정의 개인정보 보호 및 보안을 엽니다.', '아래쪽 보안 영역에서 DocPilot의 확인 없이 열기를 선택하고, 다시 나타나는 경고에서 열기를 선택합니다.', '폴더 접근 요청이 나오면 작업할 위치에 필요한 권한을 허용합니다.'],
    verify: ['macOS 메뉴 막대에 앱 이름이 DocPilot로 표시됩니다.', '시작 화면에서 프로젝트 폴더를 선택할 수 있습니다.'],
    notes: ['이 팀 내부용 빌드는 Developer ID 서명과 Apple 공증이 없어 브라우저로 내려받은 앱의 첫 실행이 Gatekeeper에 차단되는 것이 예상됩니다.', 'Control-click만으로 열린다고 가정하지 말고 시스템 설정 → 개인정보 보호 및 보안 → 확인 없이 열기 절차를 사용하세요.', '확인 없이 열기는 DocPilot을 한 번 실행해 차단된 뒤에만 표시됩니다.', '회사·학교에서 관리하는 Mac이라면 기기 관리자 정책을 확인하세요. Gatekeeper를 끄거나 터미널 명령으로 격리 속성을 제거하지 마세요.', '업데이트 후 이전 창이 남아 있으면 모든 DocPilot 창을 닫고 다시 실행하세요.'],
  ...
  'editing/source': guide({
    ...
    before: ['문서 형식이 복잡하거나 Document에서 안전한 원문 보존을 증명할 수 없는 문법이 있으면 Source를 사용하세요.'],
  ...
  'editing/markdown': guide({
    description: 'Markdown은 원문을 다루는 Source와 렌더링 결과에서 바로 편집하는 Document를 제공합니다. 안전한 원문 보존을 증명할 수 있는 문서에서만 Document 편집이 활성화됩니다.',
    outcome: '렌더링된 Markdown에 커서를 놓고 구조와 내용을 바로 편집한 뒤 원문을 안전하게 저장합니다.',
    before: ['MDX import/export, 원시 HTML, 일부 확장 문법 또는 300,000자 초과 문서는 Document가 읽기 전용으로 열릴 수 있습니다.', '읽기 전용 알림은 문서를 임의 변환하지 않기 위한 보호 동작이며 설정에서 다시 표시하도록 바꿀 수 있습니다.'],
    steps: ['문서를 열고 Document에서 원하는 위치를 클릭합니다.', '#, -, 번호, 인용 또는 표 문법을 입력하거나 도구막대를 사용해 블록을 만듭니다.', '목록에서는 Tab과 Shift+Tab으로 깊이를 조절하고, Agent Copy가 필요할 때만 해당 모드를 켭니다.', 'Cmd/Ctrl+S로 저장한 뒤 Source에서 원문의 줄바꿈과 문법이 유지되는지 확인합니다.'],
    verify: ['보기 전환 후 제목과 본문 구조가 일치합니다.', '프로젝트 안의 상대 이미지가 문서 위치 기준으로 표시됩니다.'],
    notes: ['Document가 읽기 전용인 문서는 Source와 Preview를 사용하세요.', 'HTML은 보안을 위해 Markdown Preview에서 그대로 실행되지 않습니다.'],
  ...
  'editing/asciidoc': guide({
    ...
    before: ['지원 확장자는 `.adoc`, `.asciidoc`, `.asc`입니다.', 'Markdown Document 편집과 JSON Tree는 제공하지 않습니다.'],
  ...
  'editing/json': guide({
    ...
    notes: ['유효하지 않은 JSON에는 포맷을 강제로 적용하지 마세요.', 'JSON에는 Markdown Document나 Preview 보기가 없습니다.'],
  ...
  'settings/reference': guide({
    ...
    steps: ['상단 오른쪽에서 앱 테마를 선택합니다.', '문서 도구막대의 더보기 버튼을 엽니다.', '들여쓰기, 줄 번호와 Preview 읽기 폭을 조정합니다.', 'Source, Document, Preview 또는 Tree 중 파일 형식에 맞는 모드를 선택합니다.'],
  ...
  'install/updates': guide({
    ...
    steps: ['업데이트 카드에서 버전과 세션 보존 안내를 확인합니다.', '릴리즈 노트를 읽고 업데이트 다운로드를 누릅니다.', 'SHA-256 검증이 끝나면 DMG 열기를 누릅니다.', '작업을 저장하고 DocPilot을 직접 종료한 뒤 Applications의 기존 앱을 새 버전으로 교체합니다.', '교체한 unsigned 앱이 처음 차단되면 DocPilot을 한 번 실행한 뒤 시스템 설정 → 개인정보 보호 및 보안 → 확인 없이 열기 → 열기로 최초 1회 승인합니다.'],
    verify: ['About DocPilot에 설치한 버전이 표시됩니다.', '설치한 버전과 GitHub Latest Release 버전이 같습니다.', '설치 이미지와 Dock 아이콘 바깥쪽에 검은 사각 배경이 없습니다.'],
    notes: ['공개 매뉴얼의 Download는 GitHub Latest Release의 자산을 선택합니다. 매뉴얼 자체의 배포 버전만 보고 앱 배포가 끝났다고 판단하지 마세요.', 'Developer ID 서명과 공증이 없으므로 자동 설치·자동 재시작은 지원하지 않습니다. 확인 없이 열기 절차는 설치 안내와 같습니다.', '업데이트마다 새 앱 번들이므로 macOS가 보안 예외 승인을 다시 요구할 수 있습니다.', 'Gatekeeper를 끄거나 터미널에서 격리 속성을 제거하지 마세요.'],
