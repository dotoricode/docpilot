...
const guidePages = {
  ...
  install: guide({
    description: 'macOS용 DMG를 내려받아 Applications에 설치합니다. 사이트의 Download에서 Apple Silicon 또는 Intel Mac을 선택할 수 있습니다.',
    outcome: 'DocPilot을 실행하고 폴더 선택 화면까지 도달합니다.',
    before: ['macOS에서 앱을 설치할 권한이 필요합니다.', '기존 버전을 사용 중이면 열려 있는 문서를 저장하세요.'],
    steps: ['상단 Download를 누르고 사용 중인 Mac의 칩 또는 프로세서를 선택합니다.', 'DMG를 열고 DocPilot을 Applications 폴더로 옮깁니다.', 'Applications에서 DocPilot을 한 번 실행합니다.', 'macOS가 개발자를 확인할 수 없다며 실행을 막으면 완료를 누르고 시스템 설정의 개인정보 보호 및 보안을 엽니다.', '보안 영역의 DocPilot 항목에서 확인 없이 열기를 누른 뒤, 다시 나타난 경고에서 열기를 선택합니다.', '폴더 접근 요청이 나오면 작업할 위치에 필요한 권한을 허용합니다.'],
    verify: ['macOS 메뉴 막대에 앱 이름이 DocPilot로 표시됩니다.', '시작 화면에서 프로젝트 폴더를 선택할 수 있습니다.'],
    notes: ['이 공개 빌드는 Developer ID 서명과 Apple 공증이 없습니다. 확인 없이 열기는 DocPilot을 한 번 실행해 차단된 뒤에만 표시됩니다. 설치 파일의 릴리스 버전과 GitHub Release 출처를 먼저 확인하세요.', '확인 없이 열기 항목이 없거나 회사·학교에서 관리하는 Mac이라면 기기 관리자 정책을 확인하세요. Gatekeeper를 끄거나 터미널 명령으로 격리 속성을 제거하는 방식은 안내하지 않습니다.', '업데이트 후 이전 창이 남아 있으면 모든 DocPilot 창을 닫고 다시 실행하세요.'],
  ...
  'install/updates': guide({
    ...
    steps: ['업데이트 카드에서 버전과 세션 보존 안내를 확인합니다.', '릴리즈 노트를 읽고 업데이트 다운로드를 누릅니다.', 'SHA-256 검증이 끝나면 DMG 열기를 누릅니다.', '작업을 저장하고 DocPilot을 직접 종료한 뒤 Applications의 기존 앱을 새 버전으로 교체합니다.', '교체한 앱의 실행이 차단되면 DocPilot을 한 번 실행한 뒤 시스템 설정 → 개인정보 보호 및 보안 → 확인 없이 열기에서 실행을 승인합니다.'],
    verify: ['About DocPilot에 설치한 버전이 표시됩니다.', '설치한 버전과 GitHub Latest Release 버전이 같습니다.', '설치 이미지와 Dock 아이콘 바깥쪽에 검은 사각 배경이 없습니다.'],
    notes: ['공개 매뉴얼의 Download는 GitHub Latest Release의 자산을 선택합니다. 매뉴얼 자체의 배포 버전만 보고 앱 배포가 끝났다고 판단하지 마세요.', 'Developer ID 서명과 공증이 없으므로 자동 설치·자동 재시작은 지원하지 않습니다. 확인 없이 열기 절차는 설치 안내와 같습니다.', 'Gatekeeper를 끄거나 터미널에서 격리 속성을 제거하지 마세요.'],
